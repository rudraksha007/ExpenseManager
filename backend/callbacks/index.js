...
app.post('/api/manpower', (req, res) => {
    ...
    // Calculate the Total Salary as BasicSalary + HRA
    const TotalSalary = BasicSalary + HRA;
  
    // Validate required fields
    if (!Name || !Position || !BasicSalary || !HRA || !StartDate || !ProjectNo || !ManPowerRequestedAmt || !RequestedMonth || !RequestedYear || !EmployeeID || !Role || !WorkingDays) {
      return res.status(400).json({ message: 'Required fields are missing' });
    }
  
    // SQL query to insert manpower record
    const query = `
      INSERT INTO manpower (
        Name, Position, BasicSalary, HRA, TotalSalary, StartDate, EndDate, 
        ProjectNo, ProjectTitle, ManPowerRequestedAmt, RequestedMonth, RequestedYear, 
        BillCopy, EmployeeID, Role, WorkingDays, Remark
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    db.query(query, [
      Name, Position, BasicSalary, HRA, TotalSalary, StartDate, EndDate, 
      ProjectNo, ProjectTitle, ManPowerRequestedAmt, RequestedMonth, RequestedYear, 
      JSON.stringify(BillCopy), EmployeeID, Role, WorkingDays, Remark
    ], (err, result) => {
...
app.put('/api/manpower/:id', (req, res) => {
    ...
    const query = `
      UPDATE manpower 
      SET Name = ?, Position = ?, BasicSalary = ?, HRA = ?, TotalSalary = ?, StartDate = ?, EndDate = ?, 
          ProjectNo = ?, ProjectTitle = ?, ManPowerRequestedAmt = ?, RequestedMonth = ?, RequestedYear = ?, 
          BillCopy = ?, EmployeeID = ?, Role = ?, WorkingDays = ?, Remark = ? 
      WHERE ManpowerId = ?
    `;
    ...
    db.query(query, [
      Name, Position, BasicSalary, HRA, TotalSalary, StartDate, EndDate, 
      ProjectNo, ProjectTitle, ManPowerRequestedAmt, RequestedMonth, RequestedYear, 
      JSON.stringify(BillCopy), EmployeeID, Role, WorkingDays, Remark, id
    ], (err, result) => {
